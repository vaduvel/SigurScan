...
TWILIO_ALLOW_UNSIGNED_WEBHOOKS="${TWILIO_ALLOW_UNSIGNED_WEBHOOKS:-false}"
TWILIO_PUBLIC_BASE_URL="${TWILIO_PUBLIC_BASE_URL:-}"
TWILIO_PROTECTED_CALL_FORWARD_TO="${TWILIO_PROTECTED_CALL_FORWARD_TO:-}"
ENV_VARS="${ENV_VARS:-REQUIRE_API_KEY=true,ENABLE_RATE_LIMIT=true,RATE_LIMIT_FAIL_CLOSED=true,PLAY_INTEGRITY_MODE=monitor,FAST_REPUTATION_MODE=true,FAST_REPUTATION_INCLUDE_URLHAUS=true,ENABLE_DEEP_REPUTATION_FALLBACK=true,ENABLE_URL_REPUTATION=true,ENABLE_ASF_INVESTOR_ALERTS=true,ENABLE_PHISHING_DATABASE=true,ENABLE_SCAM_BLOCKLIST_NRD=true,ENABLE_PHISHDESTROY=true,ENABLE_DNS_REPUTATION=true,ENABLE_MISTRAL_SEMANTIC_PILLAR=true,ENABLE_MISTRAL_SHADOW_ADJUDICATION=false,ENABLE_OFFER_CLAIM_WEB_CHECK=false,OPENAPI_RO_MONTHLY_BUDGET=100,HUNTER_IO_MONTHLY_BUDGET=50,URLSCAN_VISIBILITY_DEFAULT=unlisted,GOOGLE_CLOUD_VISION_LOCATION=eu,TWILIO_ALLOW_UNSIGNED_WEBHOOKS=$TWILIO_ALLOW_UNSIGNED_WEBHOOKS,TWILIO_PUBLIC_BASE_URL=$TWILIO_PUBLIC_BASE_URL,TWILIO_PROTECTED_CALL_FORWARD_TO=$TWILIO_PROTECTED_CALL_FORWARD_TO,ORCHESTRATED_CLOUD_TASKS_ENABLED=$ORCHESTRATED_CLOUD_TASKS_ENABLED,CLOUD_TASKS_PROJECT=$CLOUD_TASKS_PROJECT,CLOUD_TASKS_LOCATION=$CLOUD_TASKS_LOCATION,CLOUD_TASKS_QUEUE=$CLOUD_TASKS_QUEUE,CLOUD_TASKS_REQUEST_TIMEOUT_SECONDS=$CLOUD_TASKS_REQUEST_TIMEOUT_SECONDS,ORCHESTRATED_CLOUD_TASKS_CONTINUE_DELAY_SECONDS=$ORCHESTRATED_CLOUD_TASKS_CONTINUE_DELAY_SECONDS}"
...
if [[ -n "$TWILIO_AUTH_TOKEN_SECRET" ]]; then
  SECRETS="$SECRETS,TWILIO_AUTH_TOKEN=$TWILIO_AUTH_TOKEN_SECRET"
fi
