...
def _user_risk_level_text(risk_level: str) -> str:
    ...
    if normalized in {"unverified", "info"}:
        return "Neconfirmat"
    return "Neclar"
...
def _user_recommended_action(risk_level: str) -> str:
    ...
    if normalized in {"unverified", "info"}:
        return "Nu avem dovezi suficiente ca să confirmăm siguranța. Verificați pe canalul oficial înainte de a acționa."
...
def _orchestrated_result_is_final(job: Dict[str, Any], analysis: Dict[str, Any]) -> bool:
    ...
    return label in {"SAFE", "SUSPECT", "UNVERIFIED", "DANGEROUS"} and gate.get("is_final", True) is not False
...
async def _finalize_orchestrated_job_if_ready(job: Dict[str, Any], request: Request) -> Dict[str, Any]:
    ...
    evidence = analysis.get("evidence", {}) if isinstance(analysis.get("evidence"), dict) else {}
    gate = evidence.get("verdict_gate") if isinstance(evidence.get("verdict_gate"), dict) else {}
    if str(gate.get("label") or "").upper() == "UNVERIFIED" and gate.get("is_final", True) is False:
        if existing_result and existing_result.get("is_final", True) is not False:
...
async def ingest_intel(payload: IntelIngestRequest):
    ...
    cfx_store.seed_from_campaigns([result])
    _INTEL_STATUS.last_run_at = time.time()
    _INTEL_STATUS.last_source = payload.source_kind
    _INTEL_STATUS.entries_ingested += 1
    _INTEL_STATUS.last_error = None
...
async def moderate_intel(payload: IntelModerateRequest):
    ...
    if payload.action == "approve":
        ...
        intel = campaign_store.get(payload.intel_id) if ok else None
        if intel:
            cfx_store.seed_from_campaigns([intel])
    else:
        ok = urechea_ingester.reject_intel(payload.intel_id)
        if ok:
            cfx_store.delete(f"cf_{payload.intel_id}")
    if not ok:
...
async def urechea_status():
    ...
    return {
        ...
        "last_source": _INTEL_STATUS.last_source,
        "entries_ingested": _INTEL_STATUS.entries_ingested,
        "last_error": _INTEL_STATUS.last_error,
        "sources_count": len(urechea_ingester.sources),
        "moderation_queue_count": len(urechea_ingester.moderation_queue),
        "campaign_count": len(campaign_store.all()),
        "fingerprint_count": len(cfx_store.all()),
        "persistence": {
            "supabase_enabled": supabase_store.is_supabase_enabled(),
        },
    }
