...
def brand_has_destinations(claimed_brand: str | None) -> bool:
    canonical = _canonical_brand(claimed_brand)
    return bool(canonical and canonical in _registry()["brands_with_destinations"])
...
def match_payment_destination(
    ...
    *,
    ...
    has_brand_destinations = brand_has_destinations(canonical_claim)
    if not normalized or not validate_iban(normalized).valid_structure:
