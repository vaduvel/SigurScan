"""Provider-gate helpers extracted from ``main.py``.
...
``main.<symbol>`` functional for existing tests and callers.
"""
...
def _apply_provider_gate_verdict(
    ...
    *,
    ...
    infra_flags = main._collect_infrastructure_flags(
        analysis,
        resolved_urls,
        official_destination=official_destination,
    )
    main._augment_summary_with_infra_flags(summary, infra_flags)
    main._maybe_add_dns_reputation(summary, resolved_urls)
    ...
    web_risk_consulted = main._source_ready(summary, "google_web_risk")
    asf_investor_alerts_consulted = main._source_ready(summary, "asf_investor_alerts")
    phishing_database_consulted = main._source_ready(summary, "phishing_database")
    phishtank_consulted = main._source_ready(summary, "phishtank_online_valid")
    openphish_consulted = main._source_ready(summary, "openphish")
    urlscan_consulted = any(main._source_ready(summary, name) for name in ("urlscan", "urlscan.io"))
    sensitive_url_path = main._has_sensitive_url_path(resolved_urls)
    brand_warning = main._brand_warning_matches_text(claimed_brand, raw_text)
    official_safety_education = main._looks_like_official_safety_education(raw_text)
    direct_sensitive_request = main._has_direct_sensitive_request(raw_text)
    if official_safety_education:
        brand_warning = {"triggered": False, "matched_assets": []}
    evidence["brand_warning"] = brand_warning
    main._attach_brand_warning_summary(summary, brand_warning)
    ...
    consulted_sources = [
        ...
        if main._source_ready(summary, name)
    ]
    ...
    main._enrich_with_btr_provenance(analysis, claimed_brand, raw_text, resolved_urls)

    decision_bundle = main._build_decision_evidence_bundle(
        analysis,
        resolved_urls,
        raw_text=raw_text,
        pillars=pillars,
    ...
    gate_result = main.reduce_verdict(decision_bundle)
    return main._apply_decision_contract_result(analysis, decision_bundle, gate_result, provider_gate)
...
def _project_provider_gate_verdict(
    ...
    *,
    ...
    return {
        ...
        "provider_gate": main._deep_copy_jsonable(evidence.get("provider_gate") or {}),
        "external_intel_summary": main._deep_copy_jsonable(evidence.get("external_intel_summary") or {}),
        "brand_warning": main._deep_copy_jsonable(evidence.get("brand_warning") or {}),
    }
