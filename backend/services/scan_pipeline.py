...
async def scan_text(request: TextScanRequest):
    ...
    _main = _main_module()
    raw_text = _main._normalise_obfuscated_text((request.text or "").strip())
    _main._validate_text_input("Textul trimis", raw_text, _main.MAX_TEXT_CHARS)
    return await _main.orchestrated_engine._start_orchestrated_compat(
...
async def scan_url(request: URLScanRequest):
    ...
    _main = _main_module()
    url = _main._canonicalize_url(_main._normalise_obfuscated_text(request.url or ""))
    if not url:
...
async def scan_email(
    ...
    return await _main.orchestrated_engine._start_orchestrated_from_extraction(
        extraction,
...
async def scan_invoice_endpoint(
    ...
    if pdf_file is not None:
        ...
        _main._validate_file_upload(
            ...
            allowed_mime_types=_main.ALLOWED_PDF_MIME_TYPES,
        )
        if not file_bytes.startswith(b"%PDF-"):
            raise HTTPException(status_code=400, detail="Fișierul nu pare să fie un PDF valid.")
        pdf_annotation_urls = _main._extract_pdf_annotation_links(file_bytes)
        qr_payloads = _main._extract_pdf_qr_payloads(file_bytes)
        embedded_text = _main._extract_pdf_embedded_text(file_bytes)
        try:
        ...
        qr_payloads = _main._extract_image_qr_payloads(file_bytes)
        source_type = "image"
    ...
    if official_xml_file is not None:
        ...
        _main._validate_file_upload(
            ...
            max_bytes=_main.MAX_XML_BYTES,
            allowed_exts=_main.ALLOWED_XML_EXTS,
            allowed_mime_types=_main.ALLOWED_XML_MIME_TYPES,
        )
        try:
    ...
    normalized_sanb_attestation = _main._normalize_sanb_attestation(sanb_attestation)
    ...
    client_payment_destination = _main._invoice_payment_destination_for_client(result, invoice_gate)
