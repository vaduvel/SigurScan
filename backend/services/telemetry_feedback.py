"""Feedback evaluation, threshold sweep and trend summaries for telemetry.

Extracted from telemetry.py. Pure transforms over record lists; the optional
load of scan records uses a lazy import so this module does not depend on the
telemetry IO/path globals at import time.
"""
...
from services.telemetry_coerce import (
    _coerce_optional_bool,
    _safe_div,
    _coerce_feedback_ts,
    _coerce_feedback_bool,
    _coerce_signal_list,
...
def build_feedback_evaluation_rows(
    ...
    *,
    ...
    if scan_records is None:
        from services.telemetry import load_scan_records  # lazy import avoids import cycle
        scans = list(load_scan_records())
    else:
        scans = list(scan_records)
