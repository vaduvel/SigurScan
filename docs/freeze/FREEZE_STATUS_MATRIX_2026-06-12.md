...
- Branch: `main`
- GitHub: `origin/main`
- Current deployed code baseline after 2026-06-13 scan pipeline hardening: `e833a63`
- Deployed Cloud Run code image: `e833a63`
- Documentation-only commits may advance `origin/main` past the deployed code baseline when they only record proof.
- MoatOS PR0-PR4 PR `#11` was squash-merged into `main` as `ab5b464`; Supabase public-grant hardening was committed as `ed8f4a7`.
- Scan pipeline hardening PR `#10` was reviewed/cherry-picked and squash-merged into `main` as `e833a63`.
- Current Cloud Run revision: `sigurscan-api-00031-hxn`, traffic `100%`, image digest `sha256:9d412bb7719242563509ad94bfbc0eb826e9bd8e252767715b215d1814a6f5b6`.
- Pre-flight source HEAD before this proof-only documentation commit: `9c898b6`.
- 2026-06-13 `fa1e75c..9c898b6` delta is workflow/tooling/test/docs only: Supabase logical backup workflow/tool/test and freeze/backup documentation. No API/runtime code is ahead of Cloud Run.
...
| Local `gate/unverified-verdict` | Not visible in this pre-flight | 2026-06-13 check: physical checkout `/Users/vaduvageorge/AndroidStudioProjects/SigurScan` is on `feature/osint-intel-pipeline`; no local `gate/unverified-verdict` worktree/branch was visible. Freeze work stayed isolated in `/Users/vaduvageorge/.config/superpowers/worktrees/SigurScan/freeze-qa-2026-06-13`; do not delete old branches until UI Sonet and any gate/unverified work are explicitly resolved. |
...
| --- | --- | --- | --- |
| 1 | Cloud Run runtime | Green for current release posture, watch on URL-provider load | Live service is healthy behind `api.sigurscan.com`, min instances is `1`, request-based CPU is preserved, budget + latency + 5xx alert policies exist and now have an email notification channel. Reproducible hash-locked container is deployed as revision `sigurscan-api-00031-hxn` on code image `e833a63`, digest `sha256:9d412bb7719242563509ad94bfbc0eb826e9bd8e252767715b215d1814a6f5b6`, traffic `100%` to latest. Runtime truth from `gcloud run services describe`: `containerConcurrency=2`, `minScale=1`, `maxScale=5`, `timeout=300`. Scan pipeline hardening proof on `main`: backend `828 passed`, Android unit/debug/release exit `0`, Cloud Build `8e7a432b-b887-4153-bfe8-d54ea0c4b70a`, official health `HTTP 200` in `0.292984s`, raw health `HTTP 200` in `0.161002s`, authenticated invoice-control smoke final on poll 3 with max poll `3.357s`. Exact-N reconciliation on 2026-06-13 used `N=2` text-only scans with no URL-provider quota: official domain `2/2` finalized, `0` `5xx`, max poll `4.344s`; raw Run `2/2` finalized, `0` `5xx`, max poll `4.36s`. Older 4/8-scan probes remain historical stress probes, not the current concurrency threshold. Deploy script hardening preserves `CONCURRENCY="${CONCURRENCY:-2}"`, uses `--concurrency "$CONCURRENCY"`, and runs `update-traffic --to-latest` after deploy so live traffic cannot remain pinned to an older revision. Optional: quota-bounded URL-provider concurrency. |
...
| 3 | Supabase | Green for runtime DB path and accepted no-PITR backup posture | Remote migration list matches local migrations through `20260613121000`. Required tables exist, RLS is enabled, `anon`/`authenticated` have no direct grants on runtime tables, preview bucket `previews` is private PNG-only with 5MB limit, visual-only constraints exist, and a live scan wrote `orch_1781289050_c007425e` to `scan_jobs`. MoatOS PR0-PR4 persistence tables were applied by migration `20260613000000_create_moatos_pr0_pr4_persistence_tables.sql`; follow-up hardening migration `20260613121000_revoke_moatos_pr0_pr4_public_access.sql` revokes all direct `anon`/`authenticated` grants. Verified MoatOS tables with RLS enabled: `brand_truth_manifest`, `campaign_intel`, `campaign_fingerprint`, `circle_link`, `guardian_audit_log`, `guardian_dangerous_approval`, `call_radar_hot_cache`; direct `anon`/`authenticated` grants count is `0`. Runtime access is Data API/PostgREST, not long-lived direct DB connections; a 20-concurrent admin telemetry pressure check returned `20/20 HTTP 200` on both official and raw Run paths. Supabase CLI proof shows `pitr_enabled=false`, `walg_enabled=true`, `backups=[]`; database size is currently `26 MB`. Automated daily logical backup is active through GitHub Actions private artifact retention. Limitation remains explicit: this is daily logical backup, not PITR, and does not include Supabase Storage objects. |
| 4 | Cache/providers | Green for deterministic/provider contract, Partial for quota-heavy load | Provider smoke, single live URL-provider smoke, cache stats, and preview cache paths have proof. Live flows prove urlscan, Google Web Risk consultation, Phishing.Database, URLhaus, and offer-claim verifier behavior; `/v1/reputation/cache/stats` reads the Supabase-backed cache on Cloud Run after `45b5663`; deployed runtime is now `ed8f4a7`. 2026-06-13 reconciliation adds redirect resolver contract proof, benign live redirect proof, RDAP/ROTLD/MX/domain-risk proof, `52/52` brand registry sweep, provider fault-injection tests proving provider errors do not become `SIGUR`, and a provider timeout/error behavior table. Open: full provider load/concurrency intentionally not run to avoid quota burn; legacy expired cache rows remain as non-blocking cache hygiene. |
| 5 | Android direct infra | Green for build/config, Partial for device proof | Debug and release local config point to `https://api.sigurscan.com/`; app API keys are configured without logging their values; direct provider keys are empty in generated BuildConfig; API key interceptor sends `X-API-KEY` and stable Android UA. 2026-06-13 clean worktree proof: `:app:testDebugUnitTest :app:assembleDebug :app:assembleRelease` passed in `4m 59s`; release APK was rebuilt with ignored local signing files and verified as signed by `CN=SigurScan, OU=Mobile Security, O=SigurScan, L=Bucharest, ST=Bucharest, C=RO`, SHA-256 `bfd7991c4a7d0c349ae41235f2c0b52d77962c5a9a6729aa3410c54840168b67`. Open: physical-device proof, mobile-network proof, upload-size proof, poor-network behavior, and post-UI-merge regression. AVD exists but did not boot into ADB during this continuation. |
...
| 7 | Code consolidation | Green for current release consolidation, Partial for branch cleanup/UI | `main` is clean in the freeze main worktree and has current invoice + offer + Cloud Run + MoatOS PR0-PR4 fixes. PR `#11` was squash-merged into `main` as `ab5b464`; Supabase grant hardening was added as `ed8f4a7`; scan pipeline hardening from draft PR `#10` was integrated as `e833a63` after cherry-pick review rather than raw merge. 2026-06-13 backend full suite after scan hardening: `828 passed, 1 warning in 4.82s`. Android unit/debug/release build passes from the freeze main worktree with exit code `0`. Earlier branch audit remains valid: DeepSeek/freeze-ready/privacy hardening are integrated; old Fable/offer stage branches must not be merged raw because they would revert current `main`. Open: do not delete old branches until Sonet UI and any active `gate/unverified-verdict` work are explicitly resolved. |
| 8 | Hardening/regression | Green for automated/runtime hardening, Partial for physical/store launch | Zone 8 targeted regression passed `231` tests. Provider-degrade regression passed `54` tests. 2026-06-13 targeted deterministic/security pre-flight passed `61` tests, and `backend/test_freeze_hardening.py` passed `4/4`. Offer corpus recall covers OP-01..09 with `52 passed` and fixture distribution `OP-01..07=5 each`, `OP-08=8`, `OP-09=8`; signal recall is `26/29 = 90%`. Runtime brand sweep checked `52` brands / `94` aliases with `0` failures. Live log hygiene checked a fresh text-only scan `orch_1781345367_80b7f443`; Cloud Logging returned `10` relevant service-window entries and found zero exact matches for marker, email, phone, CNP, IBAN, card, or OTP. Admin telemetry endpoints return live `200`; dashboard HTML returns `200`; telemetry summary reports `alerts_count=0` and urlscan pending-timeout rate `0.0`. Cloud Monitoring has enabled latency and 5xx policies with email channel `6336647364895454298` attached. Controlled 5xx stack-trace proof is deferred because no safe admin-only diagnostic crash route exists. Open: full URL-provider load pack, Play-ready privacy/legal store checklist, and physical-device/mobile-network proof. |
...
- Backend full suite, 2026-06-13 pre-flight:
  - Command: `python3 -m pytest backend -q`
...
- Android unit/debug/release build, 2026-06-13 pre-flight:
  ...
  - Release signing verification: `CN=SigurScan, OU=Mobile Security, O=SigurScan, L=Bucharest, ST=Bucharest, C=RO`, SHA-256 `bfd7991c4a7d0c349ae41235f2c0b52d77962c5a9a6729aa3410c54840168b67`.
- Provider/determinism/privacy targeted pre-flight, 2026-06-13:
  ...
  - Runtime brand registry sweep result: `brands=52`, `aliases_checked=94`, `failures=0`.
- Git hygiene pre-flight, 2026-06-13:
  - Worktree: `/Users/vaduvageorge/.config/superpowers/worktrees/SigurScan/freeze-qa-2026-06-13`, branch `freeze/qa-reconciliation-2026-06-13`, tracking `origin/main`.
  - Physical checkout: `/Users/vaduvageorge/AndroidStudioProjects/SigurScan` on `feature/osint-intel-pipeline`; no mutation performed there.
  - No local `gate/unverified-verdict` worktree/branch was visible during this check; old feature branches were not deleted.
- Zone 7 branch audit:
  ...
  - The physical checkout was on `feature/osint-intel-pipeline` during the 2026-06-13 pre-flight; no local `gate/unverified-verdict` worktree/branch was visible. Freeze checks intentionally avoided mutating the physical checkout.
- Supabase remote migrations:
  - Command: `supabase migration list --linked`
  - Result: all local migrations from `20260525091000` through `20260613121000` are present remotely, including `20260613000000_create_moatos_pr0_pr4_persistence_tables.sql` and `20260613121000_revoke_moatos_pr0_pr4_public_access.sql`.
...
- Supabase MoatOS structural checks:
  - Tables present with RLS enabled: `brand_truth_manifest`, `campaign_intel`, `campaign_fingerprint`, `circle_link`, `guardian_audit_log`, `guardian_dangerous_approval`, `call_radar_hot_cache`
  - `anon`/`authenticated` direct grant count on those tables: `0`
...
2. Keep `main` as the freeze base and keep the physical checkout isolated from freeze QA. If `gate/unverified-verdict` reappears later, treat it as separate active work.
